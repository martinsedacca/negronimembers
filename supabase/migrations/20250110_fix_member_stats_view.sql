...
FROM members m
LEFT JOIN (
    -- Subquery for usage statistics
    SELECT 
        member_id,
        COUNT(*) as total_visits,
        COUNT(*) FILTER (WHERE event_type = 'purchase') as total_purchases,
        COUNT(*) FILTER (WHERE event_type = 'event') as total_events,
        SUM(amount_spent) as lifetime_spent,
        COUNT(*) FILTER (WHERE usage_date >= NOW() - INTERVAL '30 days') as visits_last_30_days,
        SUM(amount_spent) FILTER (WHERE usage_date >= NOW() - INTERVAL '30 days') as spent_last_30_days,
        COUNT(*) FILTER (WHERE usage_date >= NOW() - INTERVAL '90 days') as visits_last_90_days,
        SUM(amount_spent) FILTER (WHERE usage_date >= NOW() - INTERVAL '90 days') as spent_last_90_days,
        MAX(usage_date) as last_visit,
        AVG(amount_spent) FILTER (WHERE amount_spent > 0) as average_purchase
    FROM card_usage
    GROUP BY member_id
