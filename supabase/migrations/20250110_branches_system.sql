...
SELECT 
    ...
    COUNT(CASE WHEN cu.event_type = 'visit' THEN 1 END) as total_visits,
    COUNT(CASE WHEN cu.event_type = 'event' THEN 1 END) as total_events,
    COALESCE(SUM(cu.amount_spent), 0) as total_revenue,
    ...
    COALESCE(SUM(CASE WHEN cu.usage_date >= NOW() - INTERVAL '30 days' THEN cu.amount_spent END), 0) as revenue_last_30_days,
