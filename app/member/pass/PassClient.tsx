...
import { Download, Sparkles, Award, Loader2, CheckCircle, AlertCircle, UtensilsCrossed, MapPin, ExternalLink, X, ChevronLeft } from 'lucide-react'
        ...
        {showMenuModal && (
          ...
          >
            <motion.div
              ...
              className={`w-full bg-neutral-900 rounded-t-3xl sm:rounded-2xl overflow-hidden ${
                selectedMenuUrl 
                  ? 'h-[95vh] sm:max-w-4xl' 
                  : 'max-h-[85vh] sm:max-w-md p-6 pb-10 sm:pb-6'
              }`}
              style={{ touchAction: selectedMenuUrl ? 'auto' : 'none' }}
            >
              {selectedMenuUrl ? (
                /* PDF Viewer */
                <>
                  {/* Header with back button */}
                  <div className="flex items-center gap-3 p-4 border-b border-neutral-700 bg-neutral-900">
                    <button
                    ...
                    <div className="flex-1 min-w-0">
                      <h2 className="text-lg font-bold text-white truncate">{selectedBranchName} Menu</h2>
                    </div>
                    <button
                      onClick={() => {
                        setSelectedMenuUrl(null)
                  ...
                  {/* PDF iframe */}
                  <div className="flex-1 h-[calc(95vh-72px)] bg-neutral-800">
                    <iframe
                      src={`${selectedMenuUrl}#toolbar=0&navpanes=0`}
                      className="w-full h-full"
                      title={`${selectedBranchName} Menu`}
                    />
                  </div>
                </>
              ) : (
                        ...
                        >
                          <div className="flex items-center justify-between gap-3">
                            ...
                            {branch.menu_url ? (
                              <button
                                onClick={() => {
                                  setSelectedMenuUrl(branch.menu_url)
