...
interface CodeBenefitsModalProps {
  code: Code
  onClose: () => void
}

export default function CodeBenefitsModal({ code, onClose }: CodeBenefitsModalProps) {
  ...
  const fetchBenefits = async () => {
    try {
      // Fetch benefits specific to this code
      const { data: codeSpecific, error: error1 } = await supabase
        .from('promotions')
        .select('*')
        .contains('applicable_to', [`code:${code.code}`])
      ...
      const allBenefits = [...(codeSpecific || []), ...(allMembers || [])]
        ...
        <div className="sticky top-0 bg-neutral-800 border-b border-neutral-700 px-6 py-4 flex items-center justify-between z-10">
          <div>
            <h2 className="text-2xl font-bold text-white flex items-center gap-2">
              ...
              Benefits for Code: <span className="text-orange-500 font-mono">{code.code}</span>
            </h2>
            <p className="text-sm text-neutral-400 mt-1">{code.description}</p>
          </div>
          <button
            onClick={onClose}
          ...
          {loading ? (
            ...
            <div className="text-center py-12">
              ...
              <p className="text-neutral-400">
                This code doesn't have any benefits assigned yet.
                <br />
                Create a benefit and assign it to this code to get started.
              </p>
            </div>
          )}
        </div>
