      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-1 bg-neutral-800 border border-neutral-700 rounded-xl p-6 space-y-4">
          ...
          {savedSegments.length > 0 && (
            <div className="p-3 bg-neutral-700/50 border border-neutral-600 rounded-lg space-y-2">
              <label className="text-xs text-neutral-400 flex items-center gap-1">
                <FolderOpen className="w-3 h-3" />
                Cargar Segmento Guardado
              </label>
              <div className="flex gap-2">
                <select
                  value={selectedSegmentId}
                  onChange={(e) => {
                    const segment = savedSegments.find(s => s.id === e.target.value)
                    if (segment) loadSavedSegment(segment)
                  }}
                  className="flex-1 px-3 py-2 bg-neutral-700 text-white border border-neutral-600 rounded-lg text-sm"
                >
                ...
                {selectedSegmentId && (
                  <button
                    ...
                    className="px-2 py-1 text-neutral-400 hover:text-red-400 transition"
